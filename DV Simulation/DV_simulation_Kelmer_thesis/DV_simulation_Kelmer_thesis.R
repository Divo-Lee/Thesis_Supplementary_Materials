...
#Part 6: DV simulation study using the Kelmer dataset 
###################################################
...
# the function "clrDV()" is the DV test function in both SIEVE and clrDV R packages
...
# mean computing times, in seconds (Table )
Time_dv_20w <- matrix(NA, nrow = 4, ncol = 5)
...
colnames(Time_dv_20w) <- c("clrDV","MDSeq","diffVar","GAMLSS", "DiffDist")
Time_dv_20w 



# standard deviation of mean time (Table )
Time_sd_dv_20w <- matrix(NA, nrow = 4, ncol = 5)
...
colnames(Time_sd_dv_20w) <- c("clrDV","MDSeq","diffVar","GAMLSS", "DiffDist")
...
abline(h = 0.05, lty = 2, lwd = 1); abline(v = 0.05, lty = 2, lwd = 1)
#legend("topright", c("clrDV", "MDSeq", "diffVar", "GAMLSS", "DiffDist"),
#       pch = c(1, 95, 8, 43, 92), pt.cex = c(1.25,1,1,1,1),
#       cex= 1.25, pt.lwd = c(1.5, 1.5, 1.5, 1.5, 1.5),
#       col = c("#d95f02", "#1b9e77", "black", "#1b9e77",  "black"))
title(adj = 0, "(c)")
