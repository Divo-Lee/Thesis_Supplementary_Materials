...
#Date: 18 Jan. 2024
#R Codes for comparison of DV methods
#Part 5: DV simulation study using the Valentim dataset 
#######################################################
...
library(clrDV); library(DiffDist)
# Note that SIEVE is an extension of clrDV
# the function "clrDV()" the is DV test function in both packages
# The results on SIEVE as a DV test (clrDV package) has been published (PeerJ, 2023, Article#16126)
...
DV.test.comparison <- function(data = NULL,
  ...
  names <- c("clrDV", "MDSeq", "diffVar", "GAMLSS",  "DiffDist")
  ...
  while (i <= N.simulations) {
    ...
    FDR[5, i] <- FDR_HM
    Type_II_error[5, i] <-type_II_error_HM
...
# Mean computing time (Table )
Time_dv_d1_new_2000genes <- matrix(NA, nrow = 4, ncol = 5)
...
colnames(Time_dv_d1_new_2000genes) <- c("clrDV","MDSeq","diffVar","GAMLSS", "DiffDist")
Time_dv_d1_new_2000genes   # Average computing Times, in seconds

# standard deviation of mean time (Table )
Time_sd_dv_d1_new <- matrix(NA, nrow = 4, ncol = 5)
...
colnames(Time_sd_dv_d1_new) <- c("clrDV","MDSeq","diffVar","GAMLSS", "DiffDist")
...
legend("bottomright", c("clrDV", "MDSeq", "diffVar", "GAMLSS", "DiffDist"),
       pch = c(1, 95, 8, 43, 92), pt.cex = c(1.25,1,1,1,1),
       cex= 1.25, pt.lwd = c(1.5, 1.5, 1.5, 1.5, 1.5),
       col = c("#d95f02", "#1b9e77", "black", "#1b9e77",  "black"))
title(adj=0, "(a)")
...
abline(h = 0.05, lty = 2, lwd = 1); abline(v = 0.05, lty = 2, lwd = 1)
#legend("topleft", c("clrDV", "MDSeq", "diffVar", "GAMLSS", "DiffDist"),
#       pch = c(1, 95, 8, 43, 92), pt.cex = c(1.25,1,1,1,1),
#       cex= 1.25, pt.lwd = c(1.5, 1.5, 1.5, 1.5, 1.5),
#       col = c("#d95f02", "#1b9e77", "black", "#1b9e77",  "black"))
title(adj=0, "(d)")
